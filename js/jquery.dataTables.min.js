...
(function(h) {
    ...
    function Va(a, b, c) {
        h(a).on("click.DT", b, function(b) {            
            a.blur();
            c(b);
            if(isFinite(b.data.action)){
                page=b.data.action;
                pageselectCallback(page);
            }
            else{
                lis = $(".paginate_button");
                lis=document.getElementsByClassName('paginate_button');
                maxNumber = lis[lis.length - 2].getElementsByTagName('a')[0].innerHTML;
                for(i=0;i<lis.length;i++){
                    if(lis[i].getAttribute('class')=='paginate_button active'){
                        page_current = parseInt(lis[i].getElementsByTagName('a')[0].innerHTML);
                    }
                }
                if (b.data.action == 'next') {
                    
                    if (page_current == maxNumber) {
                        return;
                    }
                    page = page_current + 1;
                    pageselectCallback(page - 1);
                } else if (b.data.action == 'previous') {
                    if (page_current == '1') {
                        return;
                    }
                    page = page_current - 1;
